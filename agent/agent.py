...
def run_main_agent(user_question):
    ...
    csv_tool = Tool(name="csv_tool", func=csv_agent_tool,
                    description='''Use this tool when user asks for a field value from a csv file. 
    ...
    The csv file has columns that represent the company ticker, field, and quarters.
    Field name format is pascal case with spaces between words, however some fields have abbreviations, if you cannot find that field try to make all letters uppercase, it can sometimes be mixed as well like 'Basic EPS'.
