...
# session state
if "chat_history_stock" not in st.session_state:
...
# conversation
for message in st.session_state.chat_history_stock:
...
if user_query is not None and user_query != "":
    st.session_state.chat_history_stock.append(
    ...
    with st.chat_message("AI"):
        ...
        response = run_main_agent(
            user_query, st.session_state.chat_history_stock)
        ...
        st.write(response[-1].get("messages")
                 [0].content.replace("$", "\$"))
    st.session_state.chat_history_stock.append(AIMessage(content=response))
